...
boolean  AsyncWiFiManager::startConfigPortal(char const *apName, char const *apPassword) {
  ...
  setupConfigPortal();
  scannow = -1;
  unsigned long lastDisconnectedTimeStamp = millis();
  while (_configPortalTimeout == 0 || millis() < _configPortalStart + _configPortalTimeout) {
    ...
    if(WiFi.isConnected()) {
      DEBUG_WM(String("Num connected stations: ") + wifi_softap_get_station_num());
      if (wifi_softap_get_station_num() == 0 && millis() - lastDisconnectedTimeStamp < 60000) {
        WiFi.mode(WIFI_STA);
        //notify that configuration has changed and any optional parameters should be saved
        if ( _savecallback != NULL) {
          //todo: check if any custom parameters actually exist, and check if they really changed maybe
      ...
      lastDisconnectedTimeStamp = millis();
       if (connect) {
        ...
        } else if ( scannow == -1 || millis() > scannow + 10000) {
...
void AsyncWiFiManager::setLoopExtraRoutine( void (*func)(void) ) {
  _loopExtraRoutine = func;
}
